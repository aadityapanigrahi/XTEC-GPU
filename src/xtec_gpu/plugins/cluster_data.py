...
def show_dialog():
    try:
        ...
        report_error("XTEC-GPU analysis", error)
...
class XTECDialog(NXDialog):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("XTEC-GPU fit")
    ...
    def _rescale(self, threshold, Data_thresh):
        ...
        All operations stay on torch tensors (GPU when available).
        ...
        if rescale_text == "mean":
            ...
            dt = Data_thresh.float() if torch.is_tensor(Data_thresh) \
                else torch.as_tensor(Data_thresh, dtype=torch.float32)
            Rescaled_data = torch.log(1 + dt)
            Rescaled_data = Rescaled_data - torch.mean(Rescaled_data, dim=0)
        else:  # "None"
            Rescaled_data = Data_thresh
        return Rescaled_data

    def XTEC_d(self):
        """XTEC-d: direct GMM clustering via sklearn (uses GPU preprocessing)."""
        self.enable_plots(enable=False)
        ...
        masked = Mask_Zeros(self.data.nxsignal.nxvalue, device=device)
        threshold = Threshold_Background(
            masked, threshold_type=thresh_type, device=device
        )
        Data_thresh = threshold.data_thresholded      # torch tensor
        Data_ind = threshold.ind_thresholded           # torch tensor
        self.Data_thresh = _to_numpy(Data_thresh)
        self.Data_ind = _to_numpy(Data_ind)
    ...
    def XTEC_s_PA(self):
        """XTEC-s: peak-averaged GMM clustering (fully GPU via torchgmm)."""
        self.enable_plots(enable=False)
        self.nc = int(self.parameters["n_cluster"].value)
        ...
        Peak_avg = Peak_averaging(
            self.data.nxsignal.nxvalue, threshold, device=device
        )
        Data_thresh = Peak_avg.peak_avg_data           # torch tensor

        Rescaled_data = self._rescale(threshold, Data_thresh)

        # GPU GMM — stays on device via torchgmm
        Data_for_GMM = Rescaled_data.T                 # (num_peaks, num_T)
    ...
    def XTEC_label_smooth(self):
        ...
        Data_for_GMM = Rescaled_data.T                 # (num_data, num_T)

        # Build Markov matrix on GPU
        Markov_matrix = GMM_kernels.Build_Markov_Matrix(
            Data_ind, L_scale, kernel_type, unit_cell_shape, device=device
        )

        # GPU GMM with label smoothing
        clusterGMM = GMM(Data_for_GMM, self.nc)
        clusterGMM.RunEM(
    ...
    def BIC_XTEC_d(self):
        """BIC score sweep for XTEC-d (GPU preprocessing, sklearn GMM)."""
        self.enable_plots(enable=False)
    ...
    def BIC_XTEC_s(self):
        """BIC score sweep for XTEC-s (GPU preprocessing, sklearn GMM)."""
        self.enable_plots(enable=False)
        nc_min = int(self.parameters["min_nc"].value)
        nc_max = int(self.parameters["max_nc"].value)
