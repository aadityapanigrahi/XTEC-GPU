...
This script creates a comparison directory with two subfolders:

- ``cpu_reference``: outputs generated with the original KimGroup/XTEC
  NumPy implementation.
- ``gpu_cli``: outputs generated by the XTEC-GPU CLI tutorial workflow.

The GPU side is produced by invoking the CLI as a subprocess. The CPU side
directly imports the reference repo's ``xtec`` package and reproduces the
published tutorial flow, including threshold plots, first-pass clustering,
second-pass clustering, and the final pickle bundle.

Example
-------
```bash
python scripts/generate_cpu_gpu_tutorial_outputs.py \
  /data/XTEC_GPU/test_dataset/srn0_XTEC.nxs \
  -o /data/XTEC_GPU/XTEC-GPU/comparison_runs/srn0_tutorial_cpu_vs_gpu_cuda1 \
  --gpu-device cuda:1 \
  --cpu-repo /tmp/KimGroup-XTEC
```
"""
